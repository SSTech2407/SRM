...
app.use(cors());
// Allow larger JSON payloads for face preview data URLs
...
async function ensureTables(){
  ...
  }catch(e){ console.warn('ensureTables warning:', e.message); }
}
ensureTables();
...
app.get('/api/v1/embeddings', authMiddleware, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT fe.student_id, fe.embedding, s.name, s.roll FROM face_embeddings fe JOIN students s ON fe.student_id = s.id LIMIT 200');
    const out = rows.map(r => ({ student_id: r.student_id, embedding: JSON.parse(r.embedding), name: r.name || '', roll: r.roll }));
    res.json(out);
  } catch (err) {
    console.error(err);
...
// discover columns present in students table (cached)
...
async function getStudentColumns(){
  ...
  // return a Map of column -> meta for easier decisions
  const meta = new Map();
  for (const r of rows){
...
app.post('/api/v1/students', authMiddleware, async (req, res) => {
  try {
    ...
    // Auto-fill any NOT NULL columns without defaults that aren't provided,
    // to avoid ER_NO_DEFAULT_FOR_FIELD on varied schemas
    const cols = await getStudentColumns();
    for (const [name, meta] of cols.entries()){
      ...
      if (!hasDefault){
        // choose a safe fallback based on type
        const t = meta.type;
        let v = null;
    ...
    // Provide clearer client errors for common MySQL constraint failures
...
// Save/Update face preview image (data URL)
...
app.post('/api/v1/students/import', authMiddleware, async (req,res)=>{
  try{
    ...
    await pool.query('INSERT INTO students (name, roll, section, department, course, phone, year, semester, email, created_at) VALUES ' + placeholders, values.flat());
